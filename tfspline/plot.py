...
def plot_loss(spline, type='total', title='', color='r', label=None, ax=None):
    ...
    if type == 'total':
        ...
        ax.semilogy(np.linspace(0, len(spline.I_loss_values), len(spline.I_loss_values)), spline.I_loss_values, color=color, label=label)
        if title:
            ax.set_title(title)
        else:
            ax.set_title('Curvature training learning curve')
        ax.set_xlabel('epochs')
        ax.set_ylabel(r'$\ell_{E}$')

    elif type=='ck-D':
        ax.semilogy(np.linspace(0, len(spline.D_loss_values), len(spline.D_loss_values)), spline.D_loss_values, color=color, label=label)
        ax.set_yscale('log')
        if title:
            ax.set_title(title)
        else:
        ...
        ax.set_xlabel('epochs')
        ax.set_ylabel(r'$\ell_{CK}$')

    elif type=='approx':
        ax.semilogy(np.linspace(0, len(spline.e_loss_values), len(spline.e_loss_values)), spline.e_loss_values, color=color, label=label)
        ax.set_yscale('log')
        if title:
            ax.set_title(title)
        else:
            ax.set_title('Approximation (deriv 0) training learning curve')
        ax.set_xlabel('epochs')
        ...
        plot_loss(spline, type='approx', title='$\ell_2$', ax=axes[1])
        plot_loss(spline, type='ck-D', title='$\ell_{CK}$', ax=axes[2])
        plot_loss(spline, type='curvature', title='$\ell_{E}$', ax=axes[3])
        #plot_loss(spline, type='ck-d', title='$\ell_{CK}$ / Derivative')
