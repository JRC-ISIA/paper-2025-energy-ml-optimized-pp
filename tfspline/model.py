...
class Spline():
    ...
    def __init__(self, polydegree=DEGREE_DEFAULT, polynum=1, ck=2, basis='chebyshev', shift_polynomial_centers='default'):
        ...
        self.ck = ck
        self.cyclic = False # Adds difference for derivatives > 0 between first and last endpoints to ck loss and strict continuity establishment
        self.periodic = False  # Adds difference for all derivatives between first and last endpoints to ck loss and strict continuity establishment
    ...
    def _evaluate_ck_loss_at(self, y1, y2, k):
        ...
        if self.ck_regularization:
            ...
            # Shifted Chebyshev actually misses symmetry properties due to the interval [0, 1] and would need a separate factor for left and right evaluation
                ...
                loss = tf.square((y1 - y2) / self.ck_regularization_power_basis[k]) # Experiments show that this regularization method works better also for Chebyshev basis. However, SGD will not converge.
            else:
                loss = tf.square((y1 - y2))/(math.factorial(k+1))
        else:
            loss = tf.square((y1 - y2))

        regularization_free_loss = tf.divide(tf.square((y1 - y2)), (self.polynum - 1)) # regularization-free derivative specific loss
        regularization_free_total_loss = tf.square((y1 - y2)) # regularization-free total loss

        return loss, regularization_free_loss, regularization_free_total_loss

    def ck_pressure(self):
        """Cost function: "C^k-pressure"
        (= distance between segment endpoints in continuous derivatives (see self.ck))       
        If self.periodic=True, Ck-pressure between spline endpoints is calculated, too
        ...
        for k in range(self.ck + 1):
            for i in range(self.polynum - 1):
                ...
                cost, reg_free_cost, reg_free_total_cost = self._evaluate_ck_loss_at(y1, y2, k)
                total_cost += cost
                cost_d[k] += reg_free_cost # regularization-free derivative specific loss
                regularization_free_total_cost += reg_free_total_cost # regularization-free total loss

        if self.periodic or self.cyclic:
            start = 0 if self.periodic else 1
            for k in range(start, self.ck + 1):
                y1 = self._evaluate_polynomial_at_x(0, k, self.boundary_points[0],
                                                    self._polynomial_center(0))
                y2 = self._evaluate_polynomial_at_x(self.polynum - 1, k, self.boundary_points[-1],
                                                    self._polynomial_center(self.polynum - 1))
                cost, reg_free_cost, reg_free_total_cost = self._evaluate_ck_loss_at(y1, y2, k)
                total_cost += cost
                cost_d[k] += reg_free_cost # regularization-free derivative specific loss
                regularization_free_total_cost += reg_free_total_cost # regularization-free total loss
    ...
    def _apply_corrective_polynomial(self, y1, y2, a, polynum):
        # Solve resulting equation system
        b = y1 + y2
        t = np.linalg.solve(a, b)

        p1 = poly.Polynomial(t)  # corrective polynomial
        p2 = poly.Polynomial(self.coeffs[polynum].numpy())  # current polynomial
    ...
    def _establish_continuity(self):
        ...
        for i in range(self.polynum):
            ...
            # Derivatives for left boundary point
            for k in range(no_equations_per_boundary_point):
                if i == 0:
                    diff_l = 0 # Leave point unchanged
                else:
                    target = self._evaluate_polynomial_at_x(left_index, k, x1, self._polynomial_center(left_index)).numpy()  # Overtake boundary derivative value of previous segment
                    diff_l = self._evaluate_polynomial_at_x(i, k, x1, self._polynomial_center(i)) - target  # Calculate difference to current value of polynomial at boundary point
                
                y1.append(diff_l) # RHS
                a.append(self._linear_equationsystem_x_values(x1, x0, corr_poly_degree, k)) # LHS

            # Derivatives for right boundary point
            for k in range(no_equations_per_boundary_point):
                if i == self.polynum - 1:
                    diff_r = 0 # Leave point unchanged
                else:
                    target = ((self._evaluate_polynomial_at_x(i, k, x2, self._polynomial_center(i))
                            + self._evaluate_polynomial_at_x(right_index, k, x2, self._polynomial_center(right_index))) / 2).numpy()  # Calculate mean of boundary derivative values
                    diff_r = self._evaluate_polynomial_at_x(i, k, x2, self._polynomial_center(i)) - target  # Calculate difference to current value of polynomial at boundary point
                        
                y2.append(diff_r) # RHS            
                a.append(self._linear_equationsystem_x_values(x2, x0, corr_poly_degree, k)) # LHS

            c = self._apply_corrective_polynomial(y1, y2, a, i)
            self.coeffs[i] = tf.Variable(c, dtype='float64', trainable=True)  # Update coefficients

        # Cyclicity and periodicity
        if self.periodic or self.cyclic:
            y1_at_first = []
            y1_at_last = []
            y2 = []
            ...
            for k in range(no_equations_per_boundary_point):
                if not self.periodic and self.cyclic and k == 0:
                    ...
                    first = self._evaluate_polynomial_at_x(0, k, self.data_x_split[0][0], self._polynomial_center(0))
                    last = self._evaluate_polynomial_at_x(self.polynum - 1, k, self.data_x_split[-1][-1], self._polynomial_center(self.polynum - 1))
                    target = (first + last / 2).numpy()  # Calculate mean of boundary derivative values
                    y1_at_first.append(first - target) # RHS 1
                    y1_at_last.append(last - target) # RHS 2
                a_at_first.append(self._linear_equationsystem_x_values(self.data_x_split[0][0], self._polynomial_center(0), corr_poly_degree, k)) # LHS 1
                a_at_last.append(self._linear_equationsystem_x_values(self.data_x_split[-1][-1], self._polynomial_center(self.polynum - 1), corr_poly_degree, k)) # LHS 2

            # Derivatives for unchanged boundary points
            for k in range(no_equations_per_boundary_point):
                y2.append(0) # RHS 
                a_at_first.append(self._linear_equationsystem_x_values(self.data_x_split[0][-1], self._polynomial_center(0), corr_poly_degree, k)) # LHS 1
                a_at_last.append(self._linear_equationsystem_x_values(self.data_x_split[-1][0], self._polynomial_center(self.polynum - 1), corr_poly_degree, k)) # LHS 2

            c1 = self._apply_corrective_polynomial(y1_at_first, y2, a_at_first, 0)
            c2 = self._apply_corrective_polynomial(y1_at_last, y2, a_at_last, self.polynum-1)
            self.coeffs[0] = tf.Variable(c1, dtype='float64', trainable=True)  # Update coefficients
            self.coeffs[-1] = tf.Variable(c2, dtype='float64', trainable=True)
    ...
    def _revert_to_best_epoch(self):
            ...
            del self.d_loss_values[self.best_loss_index+1:]
            del self.I_loss_values[self.best_loss_index+1:]
    ...
    def fit(self, data_x, data_y, initialization='zero', overlap_segments=0,
        ...
        :param periodic: "True" --> Align derivatives at position xn with derivatives at position x0
        :param cyclic: "True" --> Align derivatives except derivative 0 at position xn with derivatives at position x0
    ...
    def get_total_loss(self):
        cost_I = self.integrate_squared_spline_acceleration()
        _, _, regularization_free_cost_D = self.ck_pressure()
        cost_e = self._sum_squared_errors()

        return tf.add(cost_I, tf.add(regularization_free_cost_D, cost_e)), cost_e, regularization_free_cost_D, cost_I
